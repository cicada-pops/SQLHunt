SHELL := /bin/zsh -eu -o pipefail

ARG := $(word 2, $(MAKECMDGOALS))
SERVICE := app

.PHONY: clean up down build logs shell manage migrate \
        makemigrations createsuperuser check test test_reset \
		update_dependencies clean_migrations

clean:
	@find . -name "*.pyc" -exec rm -rf {} \;
	@find . -name "__pycache__" -delete

clean_migrations:
	@find . -path "*/migrations/*.py" -not -name "__init__.py" -delete
	@find . -path "*/migrations/*.pyc" -delete	
up:
	docker compose --env-file ./backend/.env up --build -d

down:
	docker compose --env-file ./backend/.env down

logs:
	docker compose logs -f $(ARG)

shell:
	docker compose --env-file ./backend/.env exec $(ARG) bash

manage:
	docker compose --env-file ./backend/.env exec $(SERVICE) python manage.py $(ARG)

migrate:
	docker compose --env-file ./backend/.env exec $(SERVICE) python manage.py migrate $(ARG)

makemigrations:
	docker compose --env-file ./backend/.env exec $(SERVICE) python manage.py makemigrations

createsuperuser:
	docker compose --env-file ./backend/.env exec $(SERVICE) python manage.py createsuperuser

check:
	-ruff check .
	-ruff format --check .

lint:
	ruff check . --fix
	ruff format .

test:
	docker-compose --env-file ./backend/.env  exec -T $(SERVICE) python manage.py test $(ARG) --parallel --keepdb

test_reset:
	docker-compose --env-file ./backend/.env exec -T $(SERVICE) python manage.py test $(ARG) --parallel

update_dependencies:
	docker compose --env-file ./backend/.env down
	docker compose --env-file ./backend/.env up -d --build
