...
class CaseModelTest(TestCase):
    ...
    def test_case_invalid_closed_status(self):
        case = Case.objects.create(
            description="Test case", date_opened=date(2023, 1, 1), date_closed=date(2023, 1, 1), type='убийство', status='открыто'
        )
        with self.assertRaises(ValidationError):
            case.clean()
...
class SuspectModelTest(TestCase):
    ...
    def test_suspect_status(self):
        person = Person.objects.create(name="John Doe", date_birth=date(2000, 1, 1), description="Test description")
        case = Case.objects.create(
            description="Test case", date_opened=date(2023, 1, 1), date_closed=None, type='убийство', status='открыто'
        )
...
class ChargeModelTest(TestCase):
    ...
    def test_charge_validation(self):
        ...
        charge = Charge.objects.create(
            article=article, suspect=suspect, date_accusation=date(2023, 1, 1), status='ожидает решения'
        )
        charge.clean()
        charge.save()
    ...
    def test_charge_invalid_date(self):
        ...
        suspect.cases.add(case)
        charge = Charge.objects.create(
            article=article, suspect=suspect, date_accusation=date.today() + timedelta(days=1), status='ожидает решения'
        )
        with self.assertRaises(ValidationError):
            charge.clean()
...
class AlibiModelTest(TestCase):
    ...
    def test_alibi_invalid_status(self):
        ...
        suspect = Suspect.objects.create(person=person, status='в розыске')
        suspect.cases.add(case)
        alibi = Alibi.objects.create(status='invalid_status', case=case, description="Test description", suspect=suspect)
        with self.assertRaises(ValidationError):
            alibi.clean()
...
class CrimeSceneModelTest(TestCase):
    ...
    def test_crime_scene_validation(self):
        ...
        crime_scene = CrimeScene.objects.create(location="Test location", date=date(2023, 1, 1), case=case)
        crime_scene.clean()
        crime_scene.save()
    ...
    def test_crime_scene_invalid_date(self):
        ...
        crime_scene = CrimeScene.objects.create(location="Test location", date=date.today() + timedelta(days=1), case=case)
        with self.assertRaises(ValidationError):
            crime_scene.clean()
...
class EvidenceModelTest(TestCase):
    ...
    def test_evidence_validation(self):
        ...
        crime_scene = CrimeScene.objects.create(location="Test location", date=date(2023, 1, 1), case=case)
        evidence = Evidence.objects.create(type='физическое', description="Test description", date=date(2023, 1, 1), scene=crime_scene)
        evidence.clean()
        evidence.save()
