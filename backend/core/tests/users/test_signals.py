...
class UsersSignalTests(TransactionTestCase):
    ...
    def setUp(self):
        self.auth_user = AuthUser.objects.create_user(username='test1', password='12345678')
    ...
    def test_create_userprogress_on_case_creation(self):
        case = Case.objects.using('users').create(title='A', description='A', 
                                                  short_description='A', reward_xp=100, 
                                                  answer='A', required_xp=0)
        is_progress_exists = UserProgress.objects.using('users').filter(user=self.user, case=case).exists()
        self.assertTrue(is_progress_exists)


    def test_create_userprogress_on_user_creation(self):
        case = Case.objects.using('users').create(title='B', description='B', 
                                                  short_description='B', reward_xp=100, 
                                                  answer='B', required_xp=0)
        new_auth_user = AuthUser.objects.create_user(username='test2', password='12345678')
        new_user = User.objects.using('users').get(id=new_auth_user.id)

        progress_exists = UserProgress.objects.using('users').filter(user=new_user, case=case).exists()
        self.assertTrue(progress_exists)
        
    def test_deduct_xp_on_case_deletion(self):
        case = Case.objects.using('users').create(title='E', description='E', 
                                                  short_description='E', reward_xp=100, 
                                                  answer='E', required_xp=0)
