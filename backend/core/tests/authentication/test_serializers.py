...
class UserRegistrationSerializerTest(TestCase):
    ...
    def test_non_unique_username(self):
        User.objects.create_user(
            username="user4", email="user4@example.com", password="password-123"
        )
        data = {
            "username": "user4",
...
class PasswordResetSerializerTest(TestCase):
    ...
    def test_valid_email(self):
        User.objects.create_user(
            username="reset", email="reset@example.com", password="password-123"
        )
...
class PasswordResetConfirmSerializerTest(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(
            username="confirm", email="confirm@example.com", password="password-123"
        )
        self.uid = urlsafe_base64_encode(force_bytes(self.user.pk))
        self.token = default_token_generator.make_token(self.user)

    def test_valid_token(self):
        data = {"uid": self.uid, "token": self.token, "password": "newpassword123"}
    ...
    def test_invalid_uid(self):
        data = {"uid": "invalid", "token": self.token, "password": "newpassword123"}
        serializer = PasswordResetConfirmSerializer(data=data)
        self.assertFalse(serializer.is_valid())
        self.assertIn("non_field_errors", serializer.errors)

    def test_invalid_token(self):
        data = {"uid": self.uid, "token": "invalid", "password": "newpassword123"}
        serializer = PasswordResetConfirmSerializer(data=data)
        self.assertFalse(serializer.is_valid())
        self.assertIn("non_field_errors", serializer.errors)
