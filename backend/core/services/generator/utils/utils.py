...
from core.services.generator.data.tables import (
    ACCESSORIES,
    BUILD,
    CHARGE_STATUSES,
    CLOTHES,
...
def get_description(name, job):
    ...
    description = (
        ...
        + (
            f"Прическа: {hair_style}, {hair_color} волосы. "
            if hair_style != "лысый"
            else f"Прическа: {hair_style}. "
        )
...
def get_available_evidence_types(crime_type):
    return [
        etype for etype in EVIDENCE_CATALOG if crime_type in EVIDENCE_CATALOG[etype]
    ]
