...
from core.investigations.models import Case as InvestigationCase
from core.users.models import AvailableTable, Case
        ...
        try:
            with transaction.atomic(using="users"):
                ...
                for table_name in self.available_tables:
                    AvailableTable.objects.using("users").create(
                        case=case, table=table_name
                    )
            ...
            raise RuntimeError(
                f"Ошибка базы данных при создании кейса '{self.title}': {e}"
            ) from e

        except Exception as e:
            raise RuntimeError(
                f"Не удалось создать кейс '{self.title}': {type(e).__name__}: {e}"
            ) from e
