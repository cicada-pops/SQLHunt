    ...
    def create_investigation(self):
        with transaction.atomic(using="investigations"):
            ...
            suspect = Suspect.objects.using("investigations").create(
                person=person, status="на свободе"
            )
            ...
            suspect_ids = Suspect.objects.using("investigations").values_list(
                "person_id", flat=True
            )
            ...
            alibi_variants = [
                (
                    "подтверждено",
                    "Находился в баре «Гнездо» — подтверждено официантом и камерой наблюдения.",
                ),
                (
                    "не подтверждено",
                    "Говорит, что был дома, но не может подтвердить — ни соседей, ни звонков.",
                ),
                (
                    "ложное",
                    "Утверждает, что был в поезде, но по базе билетов его там не было.",
                ),
            ]

            for person, (status, description) in zip(
                sample(list(persons), 3), alibi_variants
            ):
                suspect = Suspect.objects.using("investigations").create(
                    person=person, status="на свободе"
                )
