...
class Person(models.Model):
    ...
    name = models.CharField(
        max_length=100, validators=[name_validator], help_text="ФИO человека."
    )
...
class Case(models.Model):
    ...
    resolution = models.CharField(
        max_length=11,
        choices=CASE_RESOLUTION_CHOICES,
        default="не раскрыто",
    ...
    def clean(self):
        ...
        if self.date_opened > today:
            raise ValidationError(
                "The opening date of the case cannot be in the future."
            )
    ...
    class Meta:
        constraints = [
            models.CheckConstraint(
                check=Q(date_closed__gte=models.F("date_opened"))
                | Q(date_closed__isnull=True),
...
class SuspectCase(models.Model):
    ...
    suspect = models.ForeignKey(
        Suspect, on_delete=models.CASCADE, help_text="подозреваемый, связанный с делом."
    )
...
class Alibi(models.Model):
    ...
    status = models.CharField(
        max_length=15,
        choices=ALIBI_STATUS,
        help_text="cтатус алиби: подтверждено, не подтверждено, ложное.",
    )
    case = models.ForeignKey(
        Case, on_delete=models.CASCADE, help_text="дело, к которому относится алиби."
    )
...
class Statement(models.Model):
    ...
    person = models.ForeignKey(
        Person, on_delete=models.CASCADE, help_text="человек, давший заявление."
    )
