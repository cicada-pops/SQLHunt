from core.services.case_loader.case_loader import load_all_cases
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--clear", action="store_true", help="Удалить все кейсы")

    def handle(self, *args, **options):
        if options["clear"]:
            self.stdout.write(
                self.style.WARNING("Очищаем все кейсы и связанные данные...")
            )

            UserCase.objects.all().delete()
            self.reset_sequences(
                ["users_case", "users_userprogress", "users_availabletable"]
            )
