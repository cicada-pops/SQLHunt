...
class SilverKey(BaseCase):
    ...
    description = (
        ...
        Но что-то пошло не так. Один из ключевых участников исчез, а в номере найдены следы борьбы и брошенный диктофон с записанным обрывком фразы: ”…если он узнает про сделку в Роттердаме — всё пойдет прахом…” 
        ...
        Все подробности в материалах дела №70. Учтите, что настоящий преступник может не входить в число подозреваемых. Ответом должно быть ID этого человека.
        """
    )
    required_xp = 0
    reward_xp = 100
