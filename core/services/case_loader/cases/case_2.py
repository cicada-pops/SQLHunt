...
class FinalMeeting(BaseCase):
    ...
    def create_investigation(self):
        with transaction.atomic(using='investigations'):
            ...
            suspect = Suspect.objects.using('investigations').create(person=criminal, status="на свободе")
            ...
            for is_charged, article in suspect_specs:
                ...
                if is_charged and article:
                    ...
                    Charge.objects.create(
                        article=article,
                        suspect=suspect,
                        date_accusation=get_random_date_between(date(2015, 12, 31), date(2020, 12, 31)),
                        status='прекращено',
                    )
