...
class InvestigationsDataGenerator:
    ...
    def generate_alibis(self):
      ...
      for case_id, suspect_id in self.case_suspects:
          ...
          if random.random() < 0.3:
              ...
              status = random.choices(['подтверждено', 'не подтверждено'], weights=[0.6, 0.4])[0]
              description = get_confirmed_alibi() if status == 'подтверждено' else get_unconfirmed_alibi()
    ...
    def generate_case_suspect(self):
        ...
        for case in self.cases:
          ...
          if len(chosen_suspects) < num_suspects:
              ...
              chosen_suspects.update(get_sample_suspects(num_suspects - len(chosen_suspects), remaining))
    ...
    def clear_all_data(self):
        tables = ['evidence', 'alibi', 'statement',
                ...
                'article', 'person', 'cases', 'suspect_cases']
